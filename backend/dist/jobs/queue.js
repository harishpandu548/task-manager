...
export async function scheduleRecurringJobs() {
    ...
    await taskQueue.upsertJobScheduler('check-overdue-tasks', { pattern: '0 9 * * *' }, {
        name: 'check-overdue-tasks',
        data: {},
    });
    // Auto-escalate CRITICAL tasks — runs every hour
    await taskQueue.upsertJobScheduler('escalate-critical-tasks', { pattern: '0 * * * *' }, {
        name: 'escalate-critical-tasks',
        data: {},
    });
    // Due date reminders — runs every hour to check for tasks due in 24h
    await notificationQueue.upsertJobScheduler('due-date-reminders', { pattern: '0 * * * *' }, {
        name: 'due-date-reminders',
        data: {},
    });
}
// Schedule a one-off reminder for a specific task
export async function scheduleDueDateReminder(taskId, dueDate) {
    ...
    if (reminderTime > new Date()) {
        ...
        await notificationQueue.add('task-due-reminder', { taskId }, { delay: reminderTime.getTime() - Date.now() });
    }
}
